...
export default function EmailValidator() {
  ...
  const handleEmailChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (lines.length > MAX_EMAILS) {
      setEmails(lines.slice(0, MAX_EMAILS).join('\n'));
      setError(`Maximum ${MAX_EMAILS.toLocaleString()} email addresses allowed.`);
    } else {
    ...
    // Update progress bar
    const emailCount = lines.length;
    const percentage = Math.min((emailCount / MAX_EMAILS) * 100, 100);
  ...
  const validateEmails = async () => {
    ...
    if (emailList.length > MAX_EMAILS) {
      setError(`Please enter no more than ${MAX_EMAILS.toLocaleString()} emails for validation.`);
      return;
    }
          ...
          <div className="mb-6">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm text-gray-500">
                {emails.split('\n').filter(line => line.trim()).length} / {MAX_EMAILS.toLocaleString()} emails
              </span>
            </div>
            ...
            <textarea
              ...
              onPaste={(e) => {
                ...
                if (lines.length > MAX_EMAILS) {
                  e.preventDefault();
                  setEmails(lines.slice(0, MAX_EMAILS).join('\n'));
                  setError(`Maximum ${MAX_EMAILS.toLocaleString()} email addresses allowed. Extra lines were removed.`);
                  setProgress(100);
                }
              }}
            />
